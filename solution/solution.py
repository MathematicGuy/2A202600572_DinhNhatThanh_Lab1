...
class OpenAIProxy:
    def __new__(cls, *args, **kwargs):
        return openai.OpenAI(*args, **kwargs)
...
def call_openai(
    ...
    end_time = time.time()
    latency = max(end_time - start_time, 0.000001)

    return (result.choices[0].message.content or "", latency)
...
def compare_models(prompt: str) -> dict:
    ...
    ai_reps = call_openai(prompt=prompt)
    mini_reps = call_openai_mini(prompt=prompt)


    cost_estimate_ai = (len(ai_reps[0].split()) / 0.75) / 1000 * COST_PER_1K_OUTPUT_TOKENS["gpt-4o"]
    # cost_estimate_mini = (len(mini_reps[0].split()) / 0.75) / 1000 * COST_PER_1K_OUTPUT_TOKENS["gpt-4o-mini"]
    response = {
        'gpt4o_response': ai_reps[0],
        'mini_response': mini_reps[0],
        'gpt4o_latency': ai_reps[1],
        'mini_latency': mini_reps[1],
        'gpt4o_cost_estimate': cost_estimate_ai
    }
...
def format_comparison_table(results: list[dict]) -> str:
    ...
    # Build header
    header = f"{'Prompt':<{col_width}} | {'GPT-4o Response':<{col_width}} | {'Mini Response':<{col_width}} | {'GPT-4o Latency':<12} | {'Mini Latency':<12}"
    separator = "-" * len(header)

    # Build rows
    rows = [header, separator]
    for result in results:
        prompt = truncate(result.get("prompt", ""), col_width)
        gpt4o_resp = truncate(result.get("gpt4o_response", ""), col_width)
        mini_resp = truncate(result.get("mini_response", ""), col_width)
        gpt4o_lat = f"{result.get('gpt4o_latency', 0):.2f}s"
        mini_lat = f"{result.get('mini_latency', 0):.2f}s"

        row = f"{prompt:<{col_width}} | {gpt4o_resp:<{col_width}} | {mini_resp:<{col_width}} | {gpt4o_lat:<12} | {mini_lat:<12}"
        rows.append(row)
